...
exports.submitAgreement = submitAgreement;
...
async function submitOnboardingForm(onboardingToken, payload) {
    ...
    await prisma_1.prisma.pointOnboarding.update({
        ...
        data: {
            ...
            status: client_1.PointOnboardingStatus.SUBMITTED,
...
async function submitAgreement(onboardingToken, payload) {
    const ob = await prisma_1.prisma.pointOnboarding.findUnique({ where: { onboardingToken } });
    if (!ob)
        throw new Error("Invalid token");
    if (ob.status !== "DRAFT")
    ...
    const agreement = await prisma_1.prisma.pointAgreement.create({
        data: {
            onboardingId: ob.id,
    ...
    await prisma_1.prisma.pointOnboarding.update({ where: { id: ob.id }, data: { status: client_1.PointOnboardingStatus.SUBMITTED, submittedAt: new Date(), signaturePath } });
...
async function approveOnboarding(id, adminUserId) {
        ...
        await prisma_1.prisma.$transaction(async (tx) => {
            for (const s of ob.services) {
                ...
                await tx.gtcPointService.upsert({
                    ...
                    update: { status: client_1.ServiceStatus.ENABLED },
                    create: { gtcPointId: point.id, serviceId: s.serviceId, status: client_1.ServiceStatus.ENABLED },
                });
            }
        });
    }
    ...
    if (!existingUser) {
        ...
        await prisma_1.prisma.pointOnboarding.update({
            where: { id: ob.id },
            data: { status: client_1.PointOnboardingStatus.APPROVED, approvedAt: new Date(), approvedByUserId: adminUserId, gtcPointId: point.id, registrationToken, tokenExpiresAt: expires },
        });
      ...
      `,
        ...
        await prisma_1.prisma.pointOnboarding.update({
            where: { id: ob.id },
            data: {
                status: client_1.PointOnboardingStatus.COMPLETED,
                approvedAt: new Date(),
...
async function declineOnboarding(id, adminUserId) {
    ...
    if (!ob)
        throw new Error("Not found");
    if (ob.status !== client_1.PointOnboardingStatus.SUBMITTED)
        throw new Error("Invalid state");
    await prisma_1.prisma.pointOnboarding.update({ where: { id: ob.id }, data: { status: client_1.PointOnboardingStatus.DECLINED, approvedByUserId: adminUserId, approvedAt: new Date() } });
...
async function resendOnboardingEmail(id) {
    ...
    if (!ob)
        throw new Error("Not found");
    const status = (ob.status || client_1.PointOnboardingStatus.DRAFT).toString().toUpperCase();
    // DRAFT -> resend onboarding link
    if (status === client_1.PointOnboardingStatus.DRAFT) {
    ...
    if (status === client_1.PointOnboardingStatus.SUBMITTED) {
        const admins = await prisma_1.prisma.user.findMany({ where: { role: "ADMIN" }, select: { id: true, email: true } });
        const owners = await prisma_1.prisma.user.findMany({ where: { role: "SECTOR_OWNER", sectorId: ob.sectorId }, select: { id: true, email: true } });
        const adminIds = admins.map((a) => a.id);
        const ownerIds = owners.map((o) => o.id);
    ...
    if (status === client_1.PointOnboardingStatus.APPROVED || status === "ACCEPTED") {
...
async function completeRegistration(registrationToken, passwordHash) {
    ...
    await prisma_1.prisma.pointOnboarding.update({ where: { id: ob.id }, data: { status: client_1.PointOnboardingStatus.COMPLETED, completedAt: new Date() } });
